...
def sample_points(box_point_masks, num_points):
    ...
    return sampled_point_ids.view(*box_point_masks.size()[:2], num_points), empty_box_masks.view(
        box_point_masks.size()[:2])


def extract_points(data, pred_center, candidate_pred_box_masks, box_point_masks, empty_box_masks, num_points,
                   upsample_rate):
    ...
    for batch_id in range(b_size):
        for obj_id in range(MAX_NUM_OBJ):
            ...
            points_in_pred_boxes = gather_operation(points_in_pred_boxes.transpose(1, 2).contiguous(),
            ...
            if gt_points.size(0) < gt_point_num:
                ...
                gt_points = gather_operation(gt_points.unsqueeze(0).transpose(1, 2).contiguous(),
                                             gt_sample_inds).transpose(1, 2).contiguous()

            gt_points = gt_points.expand(points_in_pred_boxes.size(0), gt_point_num, gt_points.size(2))

            # move to pred box center
            points_in_pred_boxes_coordinates = points_in_pred_boxes[..., :3] - pred_center[
                batch_id, pred_box_mask].unsqueeze(1).detach()
            points_in_pred_boxes = torch.cat(
                [points_in_pred_boxes_coordinates, points_in_pred_boxes[..., 3].unsqueeze(-1)], dim=-1)
            ...
            object_cls_labels.append(
                data['sem_cls_label'][batch_id][obj_id].unsqueeze(0).expand(points_in_pred_boxes.size(0)))

    if len(input_object_points):
        return torch.cat(input_object_points, dim=0), torch.cat(gt_object_points, dim=0), torch.cat(object_cls_labels,
                                                                                                    dim=0)
    else:
